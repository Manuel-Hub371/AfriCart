    ...
    async function loadRecommendations() {
      try {
        ...
        const [catRes, storeRes] = await Promise.all([
          apiFetch(`/api/products?category=${encodeURIComponent(category || "")}&limit=8`),
          storeId ? apiFetch(`/api/products?storeId=${encodeURIComponent(storeId)}&limit=8`) : Promise.resolve(null),
        ]);
