...
export function ShippingSettingsForm({ onSave }: ShippingSettingsFormProps) {
  ...
  const fetchPolicies = async () => {
    try {
      setLoading(true);
      setError(null);
      const res = await apiFetch("/api/vendor/shipping-policies");
  ...
  const handleToggleActive = async (policy: ShippingPolicyItem) => {
    try {
      ...
      const res = await apiFetch(`/api/vendor/shipping-policies/${policy.id}`, {
        method: "PATCH",
  ...
  const handleDeletePolicy = async (policyId: string) => {
    ...
    try {
      const res = await apiFetch(`/api/vendor/shipping-policies/${policyId}`, {
        method: "DELETE",
      });
