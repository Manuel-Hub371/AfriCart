...
export default function VendorMarketingDashboardPage() {
  ...
  const fetchOverview = async () => {
    try {
      ...
      const [res, prodRes] = await Promise.all([
        apiFetch("/api/vendor/marketing/overview"),
  ...
  const handleOpenEditCampaign = async (c: DashboardCampaign) => {
    ...
    try {
      const res = await apiFetch(`/api/vendor/campaigns/${c.id}`);
      if (res.ok) {
  ...
  const handleDeleteCampaign = async (id: string) => {
    ...
    try {
      const res = await apiFetch(`/api/vendor/campaigns/${id}`, { method: "DELETE" });
      if (res.ok) fetchOverview();
    } catch {
      // silently fail
    }
