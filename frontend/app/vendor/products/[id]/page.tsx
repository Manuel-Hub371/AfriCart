...
export default function ProductWorkspacePage({ params }: ProductWorkspaceProps) {
  ...
  const fetchProduct = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await apiFetch(`/api/vendor/products/${productId}`);
      if (!res.ok) {
    ...
    async function loadRelatedData() {
      try {
        const [ordersRes, reviewsRes] = await Promise.all([
          apiFetch("/api/vendor/orders"),
          apiFetch("/api/vendor/reviews"),
        ]);
        if (ordersRes.ok) {
  ...
  const handleDeleteProduct = async () => {
    ...
    try {
      const res = await apiFetch(`/api/vendor/products/${productId}`, { method: "DELETE" });
      if (res.ok) {
