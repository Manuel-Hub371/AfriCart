...
function MessagesContent() {
  ...
  useEffect(() => {
    async function loadConversations(isInitial = false) {
      try {
        ...
        const res = await apiFetch("/api/messaging/conversations?role=customer");
        if (res.ok) {
  ...
  useEffect(() => {
    ...
    async function loadMessages(silent = false) {
      try {
        const res = await apiFetch(`/api/messaging/conversations/${activeConversationId}/messages`);
        if (res.ok) {
          const data = await res.json();
          setMessages((prev) => {
            if (JSON.stringify(prev) !== JSON.stringify(data)) return data || [];
              ...
              {activeConversation ? (
                <>
                  ...
                  {attachments.length > 0 && (
                    <div className="px-3 py-2 bg-emerald-50/60 border-t border-emerald-100 flex gap-2 flex-wrap">
                  ...
                  <form onSubmit={handleSendMessage} className="p-3 bg-white border-t border-gray-200/80">
                    <div className="bg-gray-50/70 border border-gray-200/90 focus-within:border-emerald-500 focus-within:bg-white focus-within:ring-2 focus-within:ring-emerald-500/20 rounded-2xl p-2 transition-all shadow-2xs">
                      ...
                      <input
                        type="text"
                        placeholder="Type a message or attach photos/videos..."
                        value={newMessage}
                        onChange={(e) => setNewMessage(e.target.value)}
                        className="w-full px-2.5 py-1 bg-transparent focus:outline-none text-xs text-gray-900 placeholder:text-gray-400 font-medium"
                      />

                      <div className="flex items-center justify-between pt-1 border-t border-gray-100/80 mt-1">
                        <button
                          type="button"
                        ...
                        <Button
                          ...
                          className="h-8 px-4 bg-emerald-600 hover:bg-emerald-700 disabled:bg-gray-200 disabled:text-gray-400 text-white font-bold text-xs rounded-xl gap-1.5 shadow-2xs"
                        >
