...
export default function WishlistPage() {
  ...
  const handleRemove = async (itemIdOrProductId: string) => {
    try {
      const res = await apiFetch(`/api/wishlist/${itemIdOrProductId}`, {
        method: "DELETE",
      });
      if (res.ok) {
        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto w-full space-y-4">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white p-4 rounded-xl border border-gray-200/80 shadow-2xs">
            <div>
              ...
              <p className="text-xs text-gray-500 font-medium mt-0.5">
                {items.length} saved {items.length === 1 ? "item" : "items"} in your wishlist
              </p>
            </div>
            <Link href="/products">
          ...
          {isLoading ? (
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="h-52 bg-white rounded-xl border border-gray-200/80 animate-pulse"></div>
              ))}
            </div>
          ) : items.length > 0 ? (
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
              {items.map((item) => {
                ...
                return (
                  <div
                    key={item.id}
                    className="bg-white rounded-xl border border-gray-200/80 overflow-hidden hover:border-gray-300 transition-all flex flex-col justify-between"
                  >
                    <div>
                      ...
                      <div className="p-3 space-y-1">
                        <Link href={`/product/${item.product.id}`}>
                          <h3 className="font-bold text-gray-900 text-xs hover:text-emerald-600 line-clamp-2 leading-tight">
                            {item.product.name}
                          </h3>
                        </Link>
                        <p className="text-[10px] text-gray-400 font-medium truncate">Store: {item.product.storeName}</p>
                        ...
                        <div className="flex items-center justify-between pt-0.5">
                          ...
                          <div className="flex items-center gap-0.5 text-[10px]">
                            ...
                            <span className="font-bold text-gray-700">{item.product.rating || 4.8}</span>
                          </div>
                        </div>
                    ...
                    <div className="p-3 pt-0 flex gap-1.5">
                      <Button
                        size="sm"
                        className="flex-1 gap-1 h-8 text-[11px] font-bold gradient-primary text-white rounded-lg"
                      ...
                      <Button
                        size="sm"
                        variant="outline"
                        className="h-8 w-8 p-0 rounded-lg border-gray-200 text-gray-400 hover:text-red-600 hover:bg-red-50"
            ...
            <div className="text-center py-14 bg-white rounded-xl border border-dashed border-gray-200 p-6">
              ...
              <p className="text-xs text-gray-400 mb-4 max-w-xs mx-auto font-medium">
