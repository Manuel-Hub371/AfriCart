...
import { Store, Search, ExternalLink, ShieldCheck, Loader2 } from "lucide-react";
...
export default function AllVendorsPage() {
  ...
  useEffect(() => {
    async function loadStores() {
      try {
        setLoading(true);
        const res = await apiFetch("/api/admin/stores?limit=50");
        if (res.ok) {
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between border-b border-slate-800 pb-6">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-100 flex items-center gap-3">
            <Store className="w-6 h-6 text-emerald-400" /> All Vendor Stores
          </h1>
          <p className="text-xs text-slate-400 mt-1">
            Complete inventory of all registered merchant stores and operational statuses.
          </p>
        </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden shadow-xl">
        {loading ? (
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="bg-slate-950/80 text-slate-400 uppercase font-bold text-[10px] tracking-wider border-b border-slate-800">
                  ...
                  <th className="py-4 px-6">Public Visibility</th>
              ...
              <tbody className="divide-y divide-slate-800/60 font-medium">
                {stores.map((s) => (
                  <tr key={s.id} className="hover:bg-slate-800/40 transition-colors">
                    ...
                    <td className="py-4 px-6 text-slate-400">{s.category}</td>
                    <td className="py-4 px-6">
                      <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold ${
                        s.status === "ACTIVE" ? "bg-emerald-950 text-emerald-400 border border-emerald-800" : "bg-amber-950 text-amber-400 border border-amber-800"
                      }`}>
                        {s.status}
                      </span>
                    </td>
                    <td className="py-4 px-6">
                      {s.isPublic ? (
                        <span className="text-emerald-400 font-bold">Public (Live)</span>
                      ) : (
                        <span className="text-slate-500">Hidden (Private)</span>
                      )}
                    </td>
          ...
          <div className="p-12 text-center text-slate-500 text-xs">No stores registered in system.</div>
        )}
      </div>
    </div>
  );
