...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json({ storePolicies: policies.generalStore });
  } catch (err: any) {
    return NextResponse.json({ error: err?.message || "Internal server error" }, { status: 500 });
  }
}

// POST /api/vendor/store-policies
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const policy = await vendorRepository.createGeneralStorePolicy(store.id, body);
