...
function ProductsContent() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans">
      ...
      <div className="bg-gradient-to-b from-white via-emerald-50/20 to-gray-50 border-b border-gray-200 py-8">
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            ...
            <form onSubmit={handleSearchSubmit} className="relative max-w-md w-full">
              ...
              <Input
                ...
                className="pl-10 pr-20 h-12 text-sm rounded-2xl bg-white border-gray-200 shadow-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20"
              />
              {searchInput && (
                <button
                  type="button"
              ...
              <Button
                type="submit"
                size="sm"
                className="absolute right-1.5 top-1.2 -translate-y-1/2 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs rounded-xl px-3 h-9"
              >
                Search
              </Button>
            </form>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 overflow-x-auto no-scrollbar flex items-center gap-2">
            ...
            {categories.map((cat) => {
              ...
              return (
                ...
                >
                  <span>{cat.name}</span>
                  <span className={`text-[10px] font-mono px-1.5 py-0.5 rounded-full ${isSelected ? "bg-emerald-700/50 text-white" : "bg-gray-200 text-gray-600"}`}>
                    {cat.productCount || 0}
                  </span>
                </button>
              );
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 bg-white p-4 rounded-2xl border border-gray-200 shadow-xs">
          <div className="flex items-center justify-between sm:justify-start gap-4">
            {/* Mobile Filter Toggle Button */}
            <Button
              variant="outline"
              onClick={() => setMobileFilterOpen(true)}
              className="lg:hidden gap-2 rounded-xl text-xs font-bold border-gray-200"
            >
              <Filter className="h-4 w-4 text-emerald-600" />
              <span>Filters</span>
              {activeFiltersCount > 0 && (
                <span className="ml-1 bg-emerald-600 text-white rounded-full text-[10px] w-5 h-5 flex items-center justify-center font-bold">
                  {activeFiltersCount}
                </span>
              )}
            </Button>
            ...
            <div className="text-xs text-gray-600 font-medium">
              Showing <span className="font-bold text-gray-900">{productsData?.products?.length || 0}</span> of{" "}
              <span className="font-bold text-gray-900">{productsData?.total || 0}</span> Products
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 self-end sm:self-auto">
            ...
            <select
              ...
              className="h-10 px-3 rounded-xl border border-gray-200 text-xs font-semibold text-gray-800 bg-white focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 cursor-pointer"
            >
