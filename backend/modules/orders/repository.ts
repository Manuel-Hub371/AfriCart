    ...
    const order = await db.$transaction(async (tx) => {
      const created = await tx.order.create({
        data: {
          ...
          // SIMULATED PAYMENT: no gateway is called yet (MoMo/card integration is a
          // documented Phase 11 blocker). Orders are marked PAID for development;
          // do NOT treat as real money flow in production.
          paymentStatus: "PAID",
          status: "PROCESSING",
          orderItems: {
