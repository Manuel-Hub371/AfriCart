...
// PATCH /api/vendor/privacy-policies/[id]
export async function PATCH(req: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    const updated = await vendorRepository.updatePrivacyPolicy(id, store.id, body);
    if (!updated) {
...
// DELETE /api/vendor/privacy-policies/[id]
export async function DELETE(_req: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    const deleted = await vendorRepository.deletePrivacyPolicy(id, store.id);
    if (!deleted) {
      return NextResponse.json({ error: "Policy not found or unauthorized" }, { status: 404 });
    }
